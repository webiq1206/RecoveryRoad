...
export const [RetentionProvider, useRetention] = createContextHook(() => {
  ...
  const evaluateEmotionalRegulation = useCallback((checkIns: DailyCheckIn[]) => {
    ...
    const reg = { ...retention.emotionalRegulation };
    ...
    const microProgress = [...retention.microProgress];
  ...
  const evaluateTriggerReduction = useCallback((checkIns: DailyCheckIn[]) => {
    ...
    const reductionPct = baselineAvg > 0
      ? Math.round(((baselineAvg - recentAvg) / baselineAvg) * 100)
      : 0;

    const currentReduction = Math.max(0, reductionPct);

    const existingMilestones = retention.triggerMilestones.map(m => m.threshold);
    const newMilestones: TriggerReductionMilestone[] = [...retention.triggerMilestones];
    ...
    if (existingTriggerMarker) {
      ...
      existingTriggerMarker.trend = recentAvg < baselineAvg ? 'improving' : (recentAvg > baselineAvg ? 'declining' : 'stable');
  ...
  const evaluateConfidenceGrowth = useCallback((
    ...
    const markers = [...retention.confidenceMarkers];
    ...
    if (!alreadyMeasuredToday) {
      markers.push({
        ...
        title: finalScore >= 80 ? 'Deep Confidence' : finalScore >= 60 ? 'Growing Trust' : finalScore >= 40 ? 'Building Belief' : 'Planting Seeds',
    ...
    if (existingConfidenceMarker) {
      ...
      existingConfidenceMarker.trend = finalScore > previousScore ? 'improving' : (finalScore < previousScore ? 'declining' : 'stable');
  ...
  const evaluateConsistency = useCallback((checkIns: DailyCheckIn[], pledgeStreak: number) => {
    const sorted = [...checkIns].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
    ...
    if (existingConsistencyMarker) {
      ...
      existingConsistencyMarker.bestStreak = Math.max(existingConsistencyMarker.bestStreak, consistencyStreak);
    ...
    const updated = { ...retention, microProgress, lastEvaluatedAt: new Date().toISOString() };
    save(updated);
  }, [retention, save]);
  ...
  const evaluateRetentionScore = useCallback(() => {
    const loopScoreAvg = retention.loops.reduce((s, l) => s + l.score, 0) / Math.max(retention.loops.length, 1);

    const progressScores = retention.microProgress.map(m => {
      return m.targetValue > 0 ? (m.currentValue / m.targetValue) * 100 : 0;
    });
    const progressAvg = progressScores.length > 0
    ...
    const updated = { ...retention, overallRetentionScore, lastEvaluatedAt: new Date().toISOString() };
    save(updated);
  }, [retention, save]);
  ...
  const runFullEvaluation = useCallback((
    ...
    evaluateEmotionalRegulation(checkIns);
    evaluateTriggerReduction(checkIns);
    evaluateConfidenceGrowth(daysSober, checkIns, journalCount, pledgeStreak, communityInteractions);
    evaluateConsistency(checkIns, pledgeStreak);
    evaluateRetentionScore();
  }, [evaluateEmotionalRegulation, evaluateTriggerReduction, evaluateConfidenceGrowth, evaluateConsistency, evaluateRetentionScore]);
